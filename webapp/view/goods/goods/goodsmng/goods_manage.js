...
$(document).ready(function(){
	...
	//没有商品分类，提示设置经营范围
	if($('#goodsClassId').find("option").length <= 1) {
		$('#noData').show();
	}
	
	//设定返回路径
	$("#returnUrl").val($("#initPath").val()+"/GoodsController.do?method=getGoodsClassByOrg");
	...
	var tabIndex = 0;
	if($('#cutab').val()) {
		tabIndex = parseInt($("#cutab").val());
	} else {
		tabIndex = parseInt($("#currentTab").val());
	}
