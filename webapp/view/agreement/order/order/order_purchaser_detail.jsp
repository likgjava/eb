...
<script type="text/javascript" src="<%=request.getContextPath()%>/view/agreement/order/order/order_purchaser_detail.js"></script>

<input type="hidden" id="isNogoodsPrice" name="isNogoodsPrice" value="${isNogoodsPrice}"/>

<div class="formLayout form2Pa">
<form:form id="OrderBuyerDetail" method="post" modelAttribute="order">
	...
	<ul>
		...
		<li><label>供应商：</label><span id="supplierOrg">${order.supplier.orgName}</span><form:hidden path="supplier.objId"></form:hidden></li>
		...
		<c:if test="${order.contract != null}">
			<li id="contract" class="fullLine"><label>相关合同：</label>
				<a href="javascript:void(0);" onclick="OrderBuyerDetail.openContract('${order.contract.objId}');return false;">${order.contract.contractNo}</a>
			</li>
		</c:if>
	</ul>
</form:form>
...
<c:if test="${order.buyerConfirmStatus == '01' && order.supplierConfirmStatus == '01' && order.payStatus != '01' && order.useStatus == '01'}">
	<button onclick="OrderBuyerDetail.toPayView('${order.objId}','${order.orderNo}',${order.goodsTotal});"><span>网银支付</span></button>
</c:if>
<button id="draftContract" class="hidden"><span>起草合同</span></button>
<button id="revocate" class="hidden"><span>撤销提交</span></button>
