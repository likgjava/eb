...
<script type="text/javascript" src="<%=request.getContextPath()%>/view/agreement/order/order/order_supplier_confirm.js"></script>

<input type="hidden" id="isNogoodsPrice" name="isNogoodsPrice" value="${isNogoodsPrice}"/>

<div class="formLayout form2Pa">
<form:form id="OrderSupplierConfirm" method="post" modelAttribute="order">
			...
			<c:forEach var = "orderItem" items="${orderItemList}">
				...
				<tr class="goodsInfo" id="${orderItem.objId}">
					<td>
						...
						<c:choose>
							<c:when test="${orderItem.goods!=null }">
								<a href="javascript:void(0);" onclick="OrderSupplierConfirm.showDetail('${orderItem.goods.objId}');return false;" >${orderItem.goods.productName } &nbsp;${orderItem.goods.productCode }</a>
							</c:when>
							<c:otherwise>
								${orderItem.desr}
							</c:otherwise>
						...
						<c:if test="${!empty orderItem.orderGoodsGifts && fn:length(orderItem.orderGoodsGifts) > 0}">
							<a href="javascript:void(0);" title="商品礼包详情" onclick="OrderSupplierConfirm.showOrHideGoodsGift(this);return false;"><img src="view/resource/skin/sysicon/16/goods_gift.png" /></a>
						</c:if>
						<!-- 有零配件  -->
						<c:if test="${!empty orderItem.orderGoodsAccessories && fn:length(orderItem.orderGoodsAccessories) > 0}">
							<a href="javascript:void(0);" title="商品零配件详情" onclick="OrderSupplierConfirm.showOrHideGoodsAccess(this);return false;"><img src="view/resource/skin/sysicon/16/goods_access.png" /></a>
						</c:if>
					...
					<td class="money">
						...
						<c:set var="saveTotal" value="${saveTotal + (orderItem.marketPrice==null||orderItem.marketPrice==0?0:(orderItem.marketPrice - orderItem.agreePrice)* orderItem.goodsQty ) }"/>
					...
					<td class="money">
						<c:if test="${param.appType=='XYGH'}">
						<input type="text" class="paddingR" name="itmeprice" onchange="OrderSupplierConfirm.goodsPriceChange(this)" value="<fmt:formatNumber value="${orderItem.goodsPrice }" pattern="#,##0.00#" />"/>
						</c:if>
						<c:if test="${param.appType=='XEJY'}">
						<input type="hidden" class="paddingR" name="itmeprice" onchange="OrderSupplierConfirm.goodsPriceChange(this)" value="<fmt:formatNumber value="${orderItem.goodsPrice }" pattern="#,##0.00#" />"/>
...
<c:if test="${order.buyerOption!=null}">
<h4><span>采购人意见</span></h4>
<ul>
	<li class="fullLine">
		<label>采购人意见：</label>
		<span>${order.buyerOption }</span>
	</li>
</ul>
</c:if>
...
<ul>
	<li class="formTextarea"><label for="paOpinion">供应商意见：</label>
	<textarea name="supplierOption" id="supplierOption">${order.supplierOption}</textarea></li>
</ul>
</div>
	<div class="conOperation">
		<button id="confirm" type="button" onclick="OrderSupplierConfirm.sureOrder()"><span>确认</span></button>
		<button id="adjust" type="button"><span>申请调整</span></button>
		<button id="destroy" type="button"><span>作废订单</span></button>
