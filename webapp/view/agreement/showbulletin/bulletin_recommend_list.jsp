...
<div class="workNews hotProduct">
	...
	<c:if var="hasProject" test="${fn:length(recommendProjects)!= 0}">
		<ul class="center">
			<c:forEach var="recommendProject" items="${recommendProjects}">
			<li>
				<div class="goodsPic">
					<a href="javascript:void(0);" onclick="RecommendProjectSupp.showProjectBulletin('${recommendProject.project.objId}','${recommendProject.project.ebuyMethod}');return false;">
						<img src="<c:url value="AttachmentController.do?method=showImg&objId=${recommendProject.picture}&fileNameSuffix=_160*160" />">
					</a>
				</div>
				<div>
					...
					<fmt:formatDate value="${recommendProject.project.evalStartTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</div>
			</li>
			</c:forEach>
		</ul>
