...
//定义文件全局变量 处理方法名重复问题
var ContractPurchaserDetail={};
ContractPurchaserDetail.oTable;	

//查看订单详情
ContractPurchaserDetail.viewOrder=function(objId,orderNo){
	 //弹出订单详情页面
	 $.epsDialog({
	        title:'订单编号：'+orderNo,
...
$(document).ready(function(){
	...
	// 订单详细
	ContractPurchaserDetail.oTable=$('#orderList').dataTable( {
		//'bPaginate': false,//不分页
		...
		'fnDrawCallback':function(oSettings) {
			ContractPurchaserDetail.oTable.oSettings=oSettings;//很重要，将当前的oSettings对象赋给当前table的oSettgins
			$("#total").text(goodsAPI.fnSumTotal(oSettings));
		},
		'fnRowCallback': function( nRow, aData, i) {//查询完毕每行的回填事件,i代表行序号	
			$(nRow).append("<td class='center'><a href='javascript:void(0);' onclick='ContractPurchaserDetail.viewOrder(\'"+aData.objId+"\',\'"+aData.orderNo+"\');return false;' >查看</a></td>");//添加操作连接
			return nRow;
		},
