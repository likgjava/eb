...
<script type="text/javascript" src="<%=request.getContextPath()%>/view/agreement/contract/contract_supplier_draft.js"></script>

<!--任务书ID-->
<input type="hidden" id="orderIds" value="<c:out value="${param.orderIds}"/>"/>
<input type="hidden" id="cureentUserId" value="${cureentUserId }"/>
  ...
  <h4><span>合同信息</span></h4>
    <ul>
      ...
      <li><label>甲方（采购人）：</label>
      		<span id="buyerName"><%=(-1!= request.getHeader("USER-AGENT").indexOf("MSIE"))?new String(request.getParameter("buyerName").getBytes("ISO-8859-1"),"gbk"):new String(request.getParameter("buyerName").getBytes("ISO-8859-1"),"UTF-8")%></span>
      		<input id="buyerId" name="buyer.objId" type="hidden" value="<c:out value="${param.buyerId}"/>"/>
      </li>
      <li><label>乙方（供应商）：</label>
      		<span id="supplierName">${orgInfo.orgName }</span>
      		<input id="supplierId" value="${orgInfo.objId }" name="supplier.objId" type="hidden"/>
      </li>
      <li><label>生效日期：</label>
