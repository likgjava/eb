var SupplierNoGoodsBargainForm = {};

//挑选商品
function selectGood(ctrlIndex,categoryId) {
//	$.epsDialog({
//	        title:'选择商品',
//	        url:$('#initPath').val()+'/view/agreement/bargin/project/bargain/goods_select_list.jsp?ctrlIndex='+ctrlIndex+"&categoryId="+categoryId,
//	        width: 800,
//			height: 400    
//	});//换为可以从行情里挑选
	...
	$.epsDialog({
	        title:'选择商品',
	        url:$('#initPath').val()+'/view/goods/goodsprice/select_goods_from_price.jsp?ctrlIndex='+ctrlIndex+"&categoryId="+categoryId,
	        width: 800,
			height: 400    
	});
...
//判断商品是否报价过
function isBargain(index) {
	var isBargain = true;
	$('div[id=bargainSupplierListDiv'+index+']').find('input[name=myGoodsId]').each(function(i,n){
		if($(n).val()) {
			if($(n).val()==$('input[id=goodsId'+index+']').val()) {
				alert('您对该商品已报过价,请点击"重新报价"');
				isBargain = false;
			}
		}
	})
...
//提交报价
SupplierNoGoodsBargainForm.saveBargain=function(index){
	var bargain={};
	bargain.project={};
...
//重新报价
SupplierNoGoodsBargainForm.updateBargain=function(index){
...
//获取供应商的报价排名图表
SupplierNoGoodsBargainForm.loadBiddingRankChart = function(){
	if($("#selfRanking").val()=='false'){return;}
...
$(document).ready(function(){
	...
	$('a[id^=submitBargain]').click(function(){
		...
		//报价商品或说明必填校验
		if(!$('textarea[id=productName'+index+']').valid()) {
			return;
		}
		//价格校验
		...
		if(window.confirm('确定提交报价')) {
			...
			if(isBargain(index)) {
				...
				SupplierNoGoodsBargainForm.saveBargain(index);
			}
		}
	})
	...
	$('a[id^=resetSubmitBargain]').click(function(){
		...
		if(window.confirm('确定提交?')) {
			$(this).attr('disabled','disabled');
			SupplierNoGoodsBargainForm.updateBargain(index);
		}
	})
	...
	$('a[id^=resetCancelBargain]').click(function(){
		...
		//回复我之前的报价和总计
		$('input[id=myGoodsPrice'+index+']').val($('input[id=oldPrice'+index+']').val());
		var requireindex = $('input[id=myGoodsPrice'+index+']').attr('requireindex');//为了获取需求条目的数量
		$('span[id=myGoodsTotal'+index+']').text(formatAmount(parseFloat($('input[id=oldPrice'+index+']').val() * parseInt($('span[id=qty'+requireindex+']').text()),2)));
	})
	...
	$(':radio').click(function(){
		var index = $(this).attr('name').replace('goodsRadio','');
		if($(this).attr('value')=='goods') {
			$('textarea[id=productName'+index+']').val('').attr('readonly','readonly');
			selectGood(index,$(this).attr('categoryId'));
		} else {
			$('textarea[id=productName'+index+']').val('').removeAttr('readonly');
			$('input[id=goodsId'+index+']').val('');
		}
	})
	
	SupplierNoGoodsBargainForm.loadBiddingRankChart();
	//定时器-请求轮次、供应商报价条目信息等
