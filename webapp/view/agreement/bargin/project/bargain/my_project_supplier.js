		...
		BargainProjectQueryList.oTable=$('#BargainProjectQueryList').dataTable( {
			...
			'fnRowCallback': function( nRow, aData, iDisplayIndex ) {
				...
				$(nRow).find("td:eq(0)").css("min-width","0px").html('<span title="展开/关闭" class="'+ ( iDisplayIndex==0?"open":BargainProjectQueryList.openFlag ) +'Btn" id="openOrclose">&nbsp;</span>').find("span[id=openOrclose]").click(function(){
					//$("tr[addColumn=add_"+aData.objId+"]").toggle();//解决IE浏览器toggle()无效的问题
				...
				//添加按钮
				$(nRow).find("td:last").append('<a name="viewProject" href="javascript:void(0);">进入项目</a>').find("a[name=viewProject]").click(function(){
					//设置返回路径
					$("#returnUrl").val($("#initPath").val()+"/SupplierSignupController.do?method=toMyProjectSupplier");
					var showProjUrl = $("#initPath").val()+"/BargainProjectController.do?method=toProjectView&userType=supplier&projectId="+aData.objId ;
					if(aData.ebuyMethod == '05'){//议价项目
						showProjUrl = $("#initPath").val()+"/TalkProjectController.do?method=toTalkProjectDetailView&userType=supplier&objId="+aData.objId ;
					}
