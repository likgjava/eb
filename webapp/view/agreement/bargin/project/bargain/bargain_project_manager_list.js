...
BargainProjectQueryList.getTableList = function(orgId) {
	...
	if(!BargainProjectQueryList.oTable) {
		...
		BargainProjectQueryList.oTable=$('#BargainProjectQueryList').dataTable( {
			...
			'singleSelect':true,
			'checkbox':false,
			'queryColumns':'projCode,projName,buyersName,ebuyMethodCN,createTime,supplierNumber,dealTotal,saveTotal,bargainStatus',
			...
			'fnDrawCallback':function(oSettings) {	
				...
				$.each( $("#BargainProjectQueryList").find("tbody tr").find("td:last") , function( index , obj ){
					...
					if(jsonRec){
						...
						//项目编号
						addHtml += ('<tr addColumn="add_'+ $(obj).parent().attr("objid") +'" '+isOpen+'><td></td><td>项目编号：</td><td colspan="8">' + jsonRec.projCode + '</td></tr>') 
					
						//预算总金额
						addHtml += ('<tr addColumn="add_'+ $(obj).parent().attr("objid") +'" '+isOpen+'><td></td><td>预算总金额：</td><td colspan="8"><span style="color:red;">' + formatAmount(jsonRec.budgetTotalMoney,2) + '（元）</span></td></tr>') 
						...
						//报名供应商
						addHtml += ('<tr addColumn="add_'+ $(obj).parent().attr("objid") +'" '+isOpen+'><td></td><td>报名供应商：</td><td colspan="8">' + ( recSupplier!=''?recSupplier:'无' ) + '</td></tr>') 
