...
<form:form id="bulletinForm" method="post" modelAttribute="bulletin">
	...
	<ul>
		...
		<span id="number" class="eleRequired">
			<c:choose>
				<c:when test="${fn:length(bulletin.bullTitle)>0}">${fn:length(bulletin.bullTitle)}/50</c:when>
				<c:otherwise>0/50</c:otherwise>
			</c:choose>
		</span>
		</li>
	</ul>
...
$(document).ready(function(){
	...
	$("#bulletinSaveBtn").click(function(){
		...
		bulletin.auditStatus = "00";//预告的审核转台一律为未审核 
		bulletin.relStatus = '01';//发布状态为未发布
