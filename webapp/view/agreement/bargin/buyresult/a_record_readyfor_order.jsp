...
<table class="tableList" id="goodsAndOption" style="width:100%;">
	<thead>
		<tr>
	...
	<tbody>
		<c:forEach var="recordItem" items="${dealDetailList}">
		
			<!-- id累计 -->
			<c:set var="detailIds" value="${detailIds}${(detailIds==''?'':',')}${recordItem.objId}"/>
			...
			<tr>
				...
				<td>
				<c:choose>
					<c:when test="${recordItem.goods !=null }">
						...
						<!-- 算节省总数  -->
						<c:set var="saveTotal" value="${saveTotal + recordItem.requireItem.goodsQty*( recordItem.goods.referPrice - recordItem.goodsPrice )  }" />
					</c:when>
					<c:otherwise>	
						${recordItem.serviceContent}
					</c:otherwise>
				</c:choose>
				<!-- 有礼包 -->
				...
				<!-- 订单总额 -->
				<c:set var="actualTotal" value="${actualTotal+ recordItem.goodsTotal}"/>
	...
	<tfoot>
		<tr>
			<td colspan="6" align="right">
					...
					节省金额：￥<em><span id="totalAmount"><fmt:formatNumber value="${saveTotal}" pattern="#,##0.00#" /></span></em>元
					商品总金额：<strong>￥<span id="totalAmount"><fmt:formatNumber value="${actualTotal}" pattern="#,##0.00#" /></span></strong>元
			</td>
		</tr>
	</tfoot>
</table>
...
recordItemDetail.createOrderByRecord = function(){
	...
	if($("#singlePrice")==1&&itemIds.length<1){
		alert("请至少选择一个商品！");return;
	}else{
		itemIds = $("input[name=detailIds]").val().split(",");
	}
