...
<form:form id="requirementForm" method="post" modelAttribute="requirement">
	...
	<div class="conOperation ">
		<button type="button" class="largeBtn" id="pass" onclick="requirement_detail.audit('02');"><span>通过</span></button>
		<button type="button" class="largeBtn" id="nopass" onclick="requirement_detail.audit('03');"><span>不通过</span></button>
		<button type="button" class="largeBtn" id="close_gview" onclick="requirement_detail.close();"><span>关闭</span></button>
	</div> 
</form:form>

<script type="text/javascript">
var requirement_detail = {};


//关闭
requirement_detail.close = function(){
	if($("#dailogId").val()){
...
//审核
requirement_detail.audit = function(auditStatus){
	$.getJSON($('#initPath').val()+'/RequirementController.do?method=updateStatus', {objIds:$("#objId").val(),"auditStatus":auditStatus}, function(json){
		if(json.failure){
			if(json.result)alert(json.result);return;
		}else{
			alert("操作成功！");
		}
		requirement_detail.close();
