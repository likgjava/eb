var HotelList = {};
HotelList.currentTabID = "auditStatus_01";
HotelList.getOperatorStr=function(objId,auditStatus){
	var operatorHtml = "";
	operatorHtml += '<td align="center">';
	if(auditStatus == "01"){
		operatorHtml += '<a href="javascript:void(0);" onclick="HotelList.viewHotel(\''+objId+'\',\'audit\');return false;" title="审核" >审核</a>';
	}
	operatorHtml += '<a href="javascript:void(0);" onclick="HotelList.viewHotel(\''+objId+'\',\'\');return false;" title="查看酒店" >查看酒店</a>';
	operatorHtml += '<a href="javascript:void(0);" onclick="HotelList.viewGuestRoom(\''+objId+'\');return false;" title="查看客房" >查看客房</a>';
	operatorHtml += '<a href="javascript:void(0);" onclick="HotelList.viewMeetingRoom(\''+objId+'\');return false;" title="查看会议室" >查看会议室</a>';
	operatorHtml += '</td>';
	return operatorHtml;
}
...
HotelList.viewHotel=function(id,auditType){
	$('#conBody').loadPage($('#initPath').val()+"/HotelController.do?method=toShowView&objId="+id+"&auditType="+auditType);
}  

//查看客房
HotelList.viewGuestRoom=function(id){
	$('#conBody').loadPage($('#initPath').val()+"/GuestRoomController.do?method=toGuestRoomListView&objId="+id+"&viewType=view");
}

//查看会议室
HotelList.viewMeetingRoom=function(id){
	$('#conBody').loadPage($('#initPath').val()+"/MeetingRoomController.do?method=toMeetingRoomListView&objId="+id+"&viewType=view");
}  
...
HotelList.getTableList = function() {
	if(!HotelList.oTable) {
		HotelList.oTable = $('#hotelList').dataTable({
			'singleSelect' : false,
			'checkbox' : false,
			'queryColumns' : 'picture,hotelName,orgInfo.orgName,district',
			'alias' : 'picture,hotelName,orgInfo.orgName,district',
			...
			"sAjaxSource" : $('#initPath').val()+ "/HotelController.do?method=list",
...
$(document).ready(function(){
	...
	$('#returnUrl').val($('#initPath').val()+ "/HotelController.do?method=toAuditHotelList");
	...
	$("#queryHotel").click(function() {
		if($("#startDate").val() != "" || $("#endDate").val() != ""){
			...
			$(HotelList.oTable.dataTableSettings).attr("params",
					$.extend(HotelList.oTable.dataTableSettings[0].params,{"startTime":""}));
