...
ExpertInfoList.getTableList = function() {
	if(!ExpertInfoList.oTable) {
		...
		ExpertInfoList.oTable=$('#ExpertInfoList').dataTable( {
			...
			'singleSelect':true,
			'checkbox':false,
			'queryColumns':'expertInfo.name,trainingCourse,trainingOrg,beginDate,endDate',
			'alias':'expertInfo.name,trainingCourse,trainingOrg,beginDate,endDate',
			...
			'fnRowCallback': function( nRow, aData, iDisplayIndex ) {
				...
				$(nRow).append(ExpertInfoList.getOperatorStr(ExpertInfoList.currentTabID.replace("auditStatus_",""))).find("a[name=view]").click(function(){
					$.epsDialog({
						...
						height:300, 
						width:500, 
						url:$("#initPath").val()+"/view/smallscale/expert/training_detil.jsp?objId="+aData['objId']
			
					})
					return false;
				}).end().find("a[name=audit]").click(function(){
					$('#conBody').loadPage($("#initPath").val()+"/view/smallscale/expert/audit_training_form.jsp?objId="+aData['objId']);
				});
			...
			"sAjaxSource": $('#initPath').val()+'/TrainingController.do?method=list'
		});
	}else {
...
$(document).ready(function(){
	//设定返回路径
	$("#returnUrl").val($('#initPath').val()+"/view/smallscale/expert/audit_training_list.jsp");
...
<div class="conSearch">
    ...
    <ul>
      ...
      <li>
        <label  for="input01">培训课程：</label>
        <input type="text" id="trainingCourse" name="trainingCourse" value=""/>
        <input type="hidden" id="trainingCourse_op" name="trainingCourse_op" value="like"/>
      </li>
      <li>
        <label  for="input01">培训机构：</label>
        <input type="text" id="trainingOrg" name="trainingOrg" value=""/>
        <input type="hidden" id="trainingOrg_op" name="trainingOrg_op" value="like"/>
      </li>
