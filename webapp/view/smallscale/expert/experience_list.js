...
ExpertInfoList.getTableList = function() {
	if(!ExpertInfoList.oTable) {
		...
		ExpertInfoList.oTable=$('#ExpertInfoList').dataTable( {
			...
			'singleSelect':true,
			'checkbox':false,
			'queryColumns':'orgName,specialty,duty,startTime,endTime',
			'alias':'orgName,specialty,duty,startTime,endTime',
			...
			'fnRowCallback': function( nRow, aData, iDisplayIndex ) {
				...
				$(nRow).append(ExpertInfoList.getOperatorStr(ExpertInfoList.currentTabID.replace("auditStatus_",""))).find("a[name=modify]").click(function(){
					$('#conBody').loadPage($('#initPath').val()+"/ExperienceController.do?method=toCreateOrUpdate&objId="+aData['objId']);
				}).end().find("a[name=delete]").click(function(){
					
					if(window.confirm("确认要删除该任职经历吗?")){
						$.ajax({
							url:$('#initPath').val()+'/ExperienceController.do?method=remove',
							type:'POST',
			...
			"sAjaxSource": $('#initPath').val()+'/ExperienceController.do?method=list&expertInfo.objId='+$('#expertInfoId').val()
		});
	}else {
...
$(document).ready(function(){
	//设定返回路径
	$("#returnUrl").val($('#initPath').val()+"/ExperienceController.do?method=toExperienceList")
	...
	$('#addExpertInfoBtn').click(function(){
		$('#conBody').loadPage($('#initPath').val()+"/ExperienceController.do?method=toCreateOrUpdate");
	});
});
