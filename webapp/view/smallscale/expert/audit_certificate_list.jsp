...
ExpertInfoList.getTableList = function() {
	if(!ExpertInfoList.oTable) {
		...
		ExpertInfoList.oTable=$('#ExpertInfoList').dataTable( {
			...
			'singleSelect':true,
			'checkbox':false,
			'queryColumns':'expertInfo.name,titleName,certificateNo,issueUnit,acquireTime',
			'alias':'expertInfo.name,titleName,certificateNo,issueUnit,acquireTime',
			...
			'fnRowCallback': function( nRow, aData, iDisplayIndex ) {
				...
				$(nRow).append(ExpertInfoList.getOperatorStr(ExpertInfoList.currentTabID.replace("auditStatus_",""))).find("a[name=view]").click(function(){
					$.epsDialog({
						...
						height:300, 
						width:500, 
						url:$("#initPath").val()+"/view/smallscale/expert/certificate_detil.jsp?objId="+aData['objId']
			
					})
					return false;
				}).end().find("a[name=audit]").click(function(){
					$('#conBody').loadPage($("#initPath").val()+"/view/smallscale/expert/audit_certificate_form.jsp?objId="+aData['objId']);
				});
				$(nRow).find("td[name=acquireTime]").empty().append(aData['acquireTime'].substring(0,16));
				return nRow;
			},
			'params':{'auditStatus':ExpertInfoList.currentTabID.replace("auditStatus_","")},
			"sAjaxSource": $('#initPath').val()+'/CertificateController.do?method=list'
		});
	}else {
...
$(document).ready(function(){
	//设定返回路径
	$("#returnUrl").val($('#initPath').val()+"/view/smallscale/expert/audit_certificate_list.jsp")
...
<div class="conSearch">
    ...
    <ul>
      ...
      <li>
        <label  for="input01">职称名称：</label>
        <input type="text" id="titleName" name="titleName" value=""/>
        <input type="hidden" id="titleName_op" name="titleName_op" value="like"/>
      </li>
      <li>
        <label  for="input01">颁发机构：</label>
        <input type="text" id="issueUnit" name="issueUnit" value=""/>
        <input type="hidden" id="issueUnit_op" name="issueUnit_op" value="like"/>
      </li>
