...
ExpertInfoList.getTableList = function() {
	if(!ExpertInfoList.oTable) {
		...
		ExpertInfoList.oTable=$('#ExpertInfoList').dataTable( {
			...
			'singleSelect':true,
			'checkbox':false,
			'queryColumns':'titleName,certificateNo,issueUnit,acquireTime',
			'alias':'titleName,certificateNo,issueUnit,acquireTime',
			...
			'fnRowCallback': function( nRow, aData, iDisplayIndex ) {
				...
				$(nRow).append(ExpertInfoList.getOperatorStr(ExpertInfoList.currentTabID.replace("auditStatus_",""))).find("a[name=modify]").click(function(){
					$('#conBody').loadPage($('#initPath').val()+"/CertificateController.do?method=toCreateOrUpdate&objId="+aData['objId']);
				}).end().find("a[name=delete]").click(function(){
					
					if(window.confirm("确认要删除该职称信息吗?")){
						$.ajax({
							url:$('#initPath').val()+'/CertificateController.do?method=remove',
							type:'POST',
			...
			"sAjaxSource": $('#initPath').val()+'/CertificateController.do?method=list&expertInfo.objId='+$('#expertInfoId').val()
		});
	}else {
...
$(document).ready(function(){
	//设定返回路径
	$("#returnUrl").val($('#initPath').val()+"/CertificateController.do?method=toCertificateList")
	...
	$('#addExpertInfoBtn').click(function(){
		$('#conBody').loadPage($('#initPath').val()+"/CertificateController.do?method=toCreateOrUpdate");
	});
});
