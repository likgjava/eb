...
<script type="text/javascript" src="<%=request.getContextPath()%>/view/es/planform/bid/bidDetail.js"></script>
<div class="partContainers">
	...
	<form id="bidForm" method="post" enctype="multipart/form-data">
		...
		<div class="formLayout form2Pa">
			<h5><span id="bidmessage">修改<dm:out value="local__OpenTendering__submitTender_zh">投标</dm:out>信息</span></h5>
			<div>
				<table>
		     		<tr>
						   ...
						   <c:choose>
						    ...
						    <c:forEach items="${project.subProject}" var="subProject">
								...
								<c:if test='${bidPackListNum > 0}'>
									<input value="${subProject.objId }" type="checkbox" checked="checked" disabled="disabled"/> ${subProject.projName}&nbsp;
								</c:if>
								<c:if test='${bidPackListNum == 0}'>
									<input value="${subProject.objId }" type="checkbox" disabled="disabled" /> ${subProject.projName}&nbsp;
								</c:if>
							</c:forEach>
						    </c:otherwise>
						   </c:choose>
			...
			<div class="formLayout form2Pa">
				<div>
					<table>
						<tr>
							...
							<td colspan="3">
								<textarea name="bidRemark" id="bidRemark" class="maxInput"  style="width: 99%;height: 50px" disabled="disabled" readonly="readonly">${bid.bidRemark}</textarea>
				...
				<div class="conOperation">
					<button id="back_to_desktop" type="button" tabindex="18"><span>返回到桌面</span></button>
				</div>
		</div>
	</form>
</div>