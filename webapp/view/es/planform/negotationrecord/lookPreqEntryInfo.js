...
$(document).ready(function(){
    	    				...
    	    				$.getJSON($("#initPath").val()+"/AttachmentController.do?method=getObjectQuery&queryColumns=objId,viewName",{"relationId":n.attachRelaId},function(json){
    			    	    	...
    			    	    	if(null != list && '' != list && "null" != list && "[]" != list){
    			    	    		...
    			    	    		$.each(list,function(k,o){
    			    	    			...
    			    	    			fileContent += '&nbsp;';
    			    	    			fileContent += '&nbsp;';
    			    	    			fileContent += '<a href="#" id="_downFile" num="'+num+'" objId="'+o.objId+'"><span>'+o.viewName+'</span></a>';
    			    	    			fileContent += '</td>';
    			    	    			fileContent += '</tr>';
    			    	    			fileContent += '</li>';
    			    	    		})
	...
	$('button[name=closeBtn]').click(function(){
		$('#showPreqEntryDetail').find(".epsDialogClose").click();
	});
