...
// 	   (d.getSeconds()>9?d.getSeconds():"0"+d.getSeconds())+".0";
    ...
    function dothing(isdraw) {
    	var rooms =null;
    	var roomtimes =null;
    	$.getJSON($('#initPath').val()+'/MeetRoomController.do?method=getEvalBidMeetRoom',function(jsonrooms){
    	    rooms = jsonrooms;
    	    $.getJSON($("#initPath").val()+'/MeetRoomController.do?method=getPresetTime',{startTime:$("#mond").val() ,endTime:$("#sund").val()},function(json){   
        	    ...
        	    $("span[class='sysicon siEdit']").click(function(){
        	    	  $.epsDialog({
        	    	        title:'修改评标室信息',
        	    	        url:$('#initPath').val()+'/MeetRoomController.do?method=toMeetRoom2&objId='+$(this).attr("name"),
        	    	        width: '600',
        	    	        height: '200',
        	    	 	    isReload:true,
        	    	        onOpen: function(){ },
