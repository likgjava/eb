...
taskPlanList.before=function(){
	var option={"useStatus":"01",//已提交
				"departmentLinker":PlatForm.user.emp.name,//招标中心项目负责人是自己
				"order":"applyDate",
				"order_flag":true}
	...
	if(taskPlanList.currentTabID == "tabs_toTemp"){//临时 
		$("#taskPlanGrid").flexAddOptionStr({
			'<span><a href="#" class="abtn">详情</a></span>' : function(btn,rowId,obj){
				btn.click(function(){
					$("#task_plan_list").hide();
					$("#task_plan_form").empty().loadPage($("#initPath").val()+"/TaskPlanController.do?method=showDetail&objId="+rowId).show();
				}).appendTo(obj);
			},
			'<span><a href="#" class="abtn">修改</a></span>' : function(btn,rowId,obj){
				btn.click(function(){
					$("#task_plan_list").hide();
					$("#task_plan_form").empty().loadPage($("#initPath").val()+"/TaskPlanController.do?method=toUpdateTaskPlanWwain&objId="+rowId).show();
				}).appendTo(obj);
			},
		...
		$("#taskPlanGrid").flexAddOptionStr({
			...
			'<span><a href="#" class="abtn">立项</a></span>' : function(btn,rowId,obj){
				btn.click(function(){
					$("#task_plan_list").hide();
					var taskPlanId = rowId;//申报书Id
					var url = $("#initPath").val();
					url += "/ProjectController.do?method=toCreateProject&taskPlanId="+taskPlanId;
					$("#task_plan_form").show().empty().loadPage(url);
				}).appendTo(obj);
			},
		});
		...
		$("#taskPlanGrid").flexAddOptionStr({
			'<span><a href="#" class="abtn">详情</a>&nbsp;&nbsp;&nbsp;&nbsp;</span>' : function(btn,rowId,obj){
				btn.click(function(){
					$("#task_plan_list").hide();
					$('#task_plan_form').empty().loadPage($('#initPath').val()+'/TaskPlanController.do?method=showDetail&canConsign=no&objId='+rowId).show();
				}).appendTo(obj);
			}
		});
...
$(document).ready(function(){
	...
	//加载tabs
	$('#epsTabs').tabs();
	taskPlanList.currentTabID = "tabs_toFormal";//初始化时，默认当前TABID为tabs_toTemp
