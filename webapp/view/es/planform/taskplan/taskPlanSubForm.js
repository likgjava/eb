...
$(document).ready(function(){
   ...
   //选择品目 
	...
	$('#taskPlanSubSave').click(function(){
		...
		$.getJSON($('#initPath').val()+'/TaskPlanSubController.do?method=createTaskPlanSub&taskPlanId='+$("#objId").val(),{"taskPlanSubString":JSON.stringify(taskPlanSub),"preqEntryString":JSON.stringify(preqEntry)}, function(json){
			if(json.result)alert(json.result);
			if(json.failure){
				$("#taskPlanSubSave").removeAttr("disabled");
				return false;
			}
			$("#moneySub").val(Number($("#totalPrice").val())+Number($("#moneySub").val()));
			//更新TaskPlanSubsids值
			if($('#type').val() && $('#type').val()=='createProject') {
   ...
   //转换数字为金额显示
    ...
    taskPlanSubForm.showmoney = function(){
    	var a = $("#totalPrice").val();//用户输入的数字
        var re = /^[0-9]+.?[0-9]*$/;   //判断字符串是否为数字      
        //判断正整数 /^[1-9]+[0-9]*]*$/      
        if (!re.test(a)){    
         	 $("#view").html("<font color='red'>请输入数字</font>");    
              $("#totalPrice").focus();    
              return false;    
        }  
        a = parseFloat(a).toFixed(2);//把用户输入的数字精确到两位小数
