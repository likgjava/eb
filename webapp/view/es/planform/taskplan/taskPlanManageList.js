...
taskPlanList.add=function(name,grid){
	...
	$("#task_plan_list").hide();
	$("#task_plan_form").show();
	$("#task_plan_form").empty().loadPage($("#initPath").val()+"/TaskPlanController.do?method=toCreateTaskPlan");
}
	...
	if(taskPlanList.currentTabID == "tabs_toTemp"){//临时 
		$("#taskPlanGrid").flexAddOptionStr({
			...
			'<span><a href="#" class="abtn">修改</a></span>' : function(btn,rowId,obj){
				btn.click(function(){
					$("#task_plan_list").hide();
					$("#task_plan_form").empty().loadPage($("#initPath").val()+"/TaskPlanController.do?method=toUpdateTaskPlan&objId="+rowId).show();
				}).appendTo(obj);
			},
