...
taskPlanList.add=function(name,grid){
	...
	$("#task_plan_list").hide();
	$("#task_plan_form").show();
	$("#task_plan_form").empty().loadPage($("#initPath").val()+"/TaskPlanController.do?method=toCreateTaskPlanWwain");
}  

taskPlanList.importTaskPlan = function(){
	 $.epsDialog({
	        title:'导入执行计划',
	        url:$('#initPath').val()+'/view/es/planform/taskplan/importTaskPlan.jsp',
	        width: '500',
	        height: '200',
	 	    isReload:false
	});
...
taskPlanList.update=function(name,grid){
	...
	//跳转到修改页面
	$('#conBody').loadPage($('#initPath').val()+'/TaskPlanController.do?method=toUpdateTaskPlanWwain&objId='+$(grid).getSelect());
} 
...
taskPlanList.before=function(){
	var option={"useStatus":"00",
				"order":"applyDate",
				"budget.objId":PlatForm.user.orgInfo.objId,
				"order_flag":true}
	...
	if(taskPlanList.currentTabID == "tabs_toTemp"){//临时 
		$("#taskPlanGrid").flexAddOptionStr({
			...
			'<span><a href="#" class="abtn">修改</a></span>' : function(btn,rowId,obj){
				btn.click(function(){
					$("#task_plan_list").hide();
					$("#task_plan_form").empty().show().loadPage($("#initPath").val()+"/TaskPlanController.do?method=toUpdateTaskPlanWwain&objId="+rowId);
				}).appendTo(obj);
			},
...
$(document).ready(function(){
	...
	$("li a.refreshData").click(function(){
		...
		var paramValue = "";
		var buttons = [];
		var orgId = PlatForm.user.orgInfo.objId;
		if(taskPlanList.currentTabID == "tabs_toTemp"){//临时	
			...
			$('#taskPlanGrid').attr("p").url = "TaskPlanController.do?method=list&order=createTime&order_flag=true&useStatus=00&budget.objId="+orgId;
			$('#taskPlanListInfo').find('input[type=checkbox]').show();
			$('#taskPlanGrid').attr("p").checkbox=true;
		}else if(taskPlanList.currentTabID == "tabs_toFormal"){//正式
			buttons = [];
			$('#taskPlanGrid').attr("p").newp = 1;
			$('#taskPlanGrid').attr("p").url = "TaskPlanController.do?method=list&order=createTime&order_flag=true&useStatus=01&auditDetail=00&confirmStatus=00&budget.objId="+orgId;
			...
			buttons = [];
			$('#taskPlanGrid').attr("p").newp = 1;
			$('#taskPlanGrid').attr("p").url = "TaskPlanController.do?method=list&budget.objId="+orgId+"&useStatus=";
			$('#taskPlanListInfo').find('input[type=checkbox]').hide();
			$('#taskPlanGrid').attr("p").checkbox=false;
		}
