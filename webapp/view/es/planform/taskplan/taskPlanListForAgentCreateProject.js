...
taskPlanList.before=function(){
	var option={
				"useStatus":"00",
				"agentLeader":PlatForm.user.emp.name,//招标中心项目负责人是自己
				"order":"applyDate",
				"order_flag":true}
...
//			}
		...
		$("#taskPlanGrid").flexAddOptionStr({
			...
			'<span><a href="#" class="abtn">立项</a></span>' : function(btn,rowId,obj){
				btn.click(function(){
					$("#task_plan_list").hide();
					$("#task_plan_form").empty().show().loadPage($("#initPath").val()+"/ProjectController.do?method=toCreateProjectForAgent&taskPlanId="+rowId);
				}).appendTo(obj);
			}
		});
