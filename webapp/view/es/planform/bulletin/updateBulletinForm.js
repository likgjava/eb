...
//	if(better_time($("#signUpSTime").val(),$("#signUpETime").val())<20){alert('报名开始时间与报名结束时间最少要相差20天！');return false;}
	...
	if($("#evalStartTime").val()<=$("#openBidTime").val()){
		alert('评标时间应大于开标时间！');
		return false;
	}
...
function validRemarkLength(){
	var msg = "";
	var flag = true;
	$("textarea[name='remark']").each(function(i){
		if($(this).val().length>250){
			msg +='第'+(i+1)+'条指标的说明字数不能超1过250个！\n';
		}
	});
	if(msg!=""){
		alert(msg);
...
$(document).ready(function(){
		...
		if(bulletinForm.checkTime()){
			if(window.confirm("确认保存?"))
			{
				...
				$("a[name='savesignUpCondFactor']").each(function(i,n){
					...
					if(obj.factorName==''){
						alert("请输入指标名称！");return false;
					}
					jsonContracts.push(obj);
				})
			jsonObj.contractStr = JSON.stringify(jsonContracts);
			$.getJSON($('#initPath').val()+'/PurBulletinController.do?method=saveUpdateDraftPurBulletin',jsonObj,function(json){
				if(json.result)alert(json.result);if(json.failure)return;
	...
	$('#bulletinSubmit').click(function(){
		...
		if(bulletinForm.checkTime()){
			...
			jsonObj.contractStr = JSON.stringify(jsonContracts);
			$.getJSON($('#initPath').val()+'/PurBulletinController.do?method=saveUpdateSubmitPurBulletin',jsonObj, function(json){
				if(json.result)alert(json.result);if(json.failure)return;
				var workFlowTaskId = $('[id=auditTaskId]').val();
				if($("#projectTaskId") && $("#projectTaskId").val() != "" &&$("#projectTaskId").val() !=null && $("#projectTaskId").val()!= undefined){
					planTemplateTask.clickMethod($("#projectTaskId").val()+"");
