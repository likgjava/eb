var mydesktop_supervise = {};
	...
	mydesktop_supervise.getPublicitybulletin = function(){
		...
		$('#superviseTabDiv').loadPage($('#initPath').val()+'/ResultPublicityController.do?method=toResultPublicity&projectId='+$('#projectId').val());
	}
	//获取采购公告
	mydesktop_supervise.getPurBulletin = function(){
		...
		if(agent!=""){
			$('#superviseTabDiv').loadPage($('#initPath').val()+'/BulletinController.do?method=toDraftPurBulletin&projectId='+$('#projectId').val());
		}else if(supervise!=""){
			$('#superviseTabDiv').loadPage($('#initPath').val()+'/BulletinController.do?method=toPurBulletinRead&projectId='+$('#projectId').val());
		}
	}
