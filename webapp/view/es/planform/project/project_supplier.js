...
$(document).ready(function(){
	...
	$("#testBtn").click(function(){
		var projId = $("#projectId").val();
		  $.epsDialog({
		        title:'报名',
		        url:$('#initPath').val()+'/SignUprecordExtController.do?method=toIndexPage&projectId='+projId,
		        width: '700',
		        height: '450',
		 	    isReload:true,
		        onOpen: function(){ },
...
//	$("#tabform5").hide();//屏蔽投标按钮
	...
	$("#tabform4").click(function(){
		$("#tabValue").val("4");
		$("#tabform").empty().loadPage($('#initPath').val()+'/SignUprecordController.do?method=toViewSignupPageTab&fromType=fromTab&projectId='+$("#projectId").val());
	...
	$("#tabform5").click(function(){
		$("#tabValue").val("5");	
		$("#tabform").empty().loadPage($('#initPath').val()+'/BidController.do?method=toSupplierBidTab&projectId='+$("#projectId").val());
	});
	...
	//项目补充公告
	$("#tabform14").click(function(){
		$("#tabValue").val("14");	
		$("#tabform").empty().loadPage($('#initPath').val()+'/ProjectController.do?method=toViewProjectOthrAttcInfo&attBizId='+$("#projectId").val());
	});
	
	//录入标书费
	$('#tabform15').click(function(){
		$("#tabform").empty().loadPage($('#initPath').val()+'/DosBuyRecordController.do?method=toDosBuyRecordListForSupplierList&projectId='+$("#projectId").val());
	});

	//录入保证金
	$('#tabform16').click(function(){
		$("#tabform").empty().loadPage($('#initPath').val()+'/BailRecordController.do?method=toBailRecordListForSupplier&projectId='+$("#projectId").val());
	});
