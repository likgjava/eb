	...
	<h5 id="reportResults"><span class="switch  left11" style="float:left; width:80%">招标项目详细信息 </span>   
    	...
    	<ul>
			...
			<li class="short">
				...
				<span><fmt:formatNumber value="${project.bail}" type="currency"/></span>
			</li>
			</c:if>
	...
	<div class="operationLog"><h5 id="reportResults"><span class="switch  left11">规则： </span></h5></div>
    	<ul>
			...
			<li>
				...
				<span  style="margin-top:3px;"><input type="radio"  value="1" checked="checked"<c:if test="${projectRule.ruleAnonymous=='1'}">checked="checked"</c:if> id="ruleAnonymous0" name="ruleAnonymous" disabled="disabled"/></span>
				<span style="margin-top:0px">是</span>
				<span  style="margin-top:3px;"><input type="radio" value="0" <c:if test="${projectRule.ruleAnonymous=='0'}">checked="checked"</c:if> id="ruleAnonymous1" name="ruleAnonymous" disabled="disabled"/></span>
	...
	<tbody>
		<c:forEach items="${subProjectList}" var="subProject" varStatus="i">
			<tr>
				...
				<td>
				<span><fmt:formatNumber value="${subProject.bail}" type="currency"/></span>
				</td>
				<c:if test="${projProcessRule.resValue==true}">
					<td>
					<span>${subProject.purDocPrice}</span>
					</td>
				</c:if>
			</tr>
		</c:forEach>
