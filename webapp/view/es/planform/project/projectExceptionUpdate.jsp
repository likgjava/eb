		...
		<form id="puaseProjectForm" method="post">
			<h5><span>暂停项目处理</span></h5>
				...
				<li>
					...
					<span><fmt:formatNumber value="${projectExceptionApply.project.budgetTotalMoney}" type="currency"></fmt:formatNumber></span>
				</li>
				<li>
				...
				<li>
					...
					<input type="radio" name="reCreateProj" value="<%=ProjExceptionApplyEnum.NORECREATEPROJECT%>" <c:if test="${projectExceptionApply.reCreateProj=='05'}">checked="checked"</c:if>/>否
				</li>
...
$(document).ready(function(){
	...
	$('#saveBtn').click(function(){//保存
		...
		if($('#puaseProjectForm').valid()){
			$.getJSON($('#initPath').val()+'/ProjectExceptionApplyController.do?method=saveUpdateExceptionApply',formToJsonObject('puaseProjectForm'),function(json){
				if(json.result)alert(json.result);if(json.failure)return;
				$('#conBody').loadPage($('#initPath').val()+'/view/es/planform/project/projectExceptionApplyList.jsp');  	
	    	});
		}
	});
	...
	$('#submitBtn').click(function(){//提交
		...
		if($('#puaseProjectForm').valid()){
			$.getJSON($('#initPath').val()+'/ProjectExceptionApplyController.do?method=saveSubmitUpdateExceptionApply',formToJsonObject('puaseProjectForm'),function(json){
				if(json.result)alert(json.result);if(json.failure)return;
				$('#conBody').loadPage($('#initPath').val()+'/view/es/planform/project/projectExceptionApplyList.jsp');  	
	    	});
		}
