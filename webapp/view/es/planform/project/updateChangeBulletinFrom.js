...
$(document).ready(function(){
	...
	$('#bulletinSave').click(function(){
		...
		{
			$.getJSON($('#initPath').val()+'/ChangebulletinController.do?method=saveUpdateChangeBulletin', formToJsonObject('variationForm'), function(json){
	...
	$('#bulletinSubmit').click(function(){
		...
		{
		$.getJSON($('#initPath').val()+'/ChangebulletinController.do?method=submitUpdateChangeBulletin', formToJsonObject('variationForm'), function(json){
			if(json.result)alert(json.result);if(json.failure)return;
			 alert("提交成功！");
			 $('#returnBtn').click();
		});
	...
	$('#returnBtn').click(function(){//返回
		$('#conBody').loadPage($('#initPath').val()+'/view/es/planform/bulletin/viewChangeBulletinList.jsp');
	});
});
