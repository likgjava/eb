...
craeteProjForTaskPlanSub.saveOrSumbit=function(){
	$('#craeteProjForTaskPlanSubForm').ajaxSubmit({
		url:$('#initPath').val()+"/RecordFormController.do?method=saveRecordFormCq",
		dataType:'json',
		success:function(json){
			if(json.failure)alert(json.result);if(json.failure)return;
			$('#conBody').loadPage($('#initPath').val()+'/TaskPlanController.do?method=toCQJYTaskPlanListForCreateProj');
		},
		error:function(msg){
			alert(msg);
		}
