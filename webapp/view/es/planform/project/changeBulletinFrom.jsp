...
<script type="text/javascript" src="<%=request.getContextPath()%>/view/es/planform/project/changeBulletinFrom.js"></script>       
<div class="partContainers" id="variationDivId">
<!-- 隐藏数据 因为该页面为公有业页，若不是弹出层，则需要重新跳转 -->
<input type="hidden" id="_projectId" value="${param.projectId}">
<form id="variationForm" method="post">
<input type="hidden" name="signUpSTime" id="signUpSTime"  readonly="readonly" value="${fn:substring(projectRule.signUpSTime,0,19)}" />
<input type="hidden" name="signUpETime" id="signUpETime" 	readonly="readonly" value="${fn:substring(projectRule.signUpETime,0,19)}" />
<input type="hidden" name="tenderStartTime" id="tenderStartTime"  readonly="readonly" value="${fn:substring(projectRule.submitStTime,0,19)}" />
<input type="hidden" name="tenderEndTime" id="tenderEndTime"  readonly="readonly" value="${fn:substring(projectRule.submitETime,0,19)}" />
<input type="hidden" name="openBidTime" id="openBidTime"  readonly="readonly" value="${fn:substring(projectRule.openBidSDate,0,19)}"/>
<input type="hidden" name="evalStartTime" id="evalStartTime"   readonly="readonly" value="${fn:substring(projectRule.evalSTime,0,19)}"/>
