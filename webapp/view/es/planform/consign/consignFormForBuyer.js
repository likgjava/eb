...
//	$('input[name=effectiveEndTime]').val('');
	...
	$('#consignSave').click(function(){
		...
		{
			$.getJSON($('#initPath').val()+'/ConsignController.do?method=createConsignForBuyer', formToJsonObject('consignForm'), function(json){
	...
	$('#consignSubmit').click(function(){
		...
		{
			$.getJSON($('#initPath').val()+'/ConsignController.do?method=submitConsignForBuyer', formToJsonObject('consignForm'), function(json){
				if(json.result)alert(json.result);if(json.failure)return;
				//跳转到上一个页面
				$('button[name=historyBackBtn]').click();
			});
