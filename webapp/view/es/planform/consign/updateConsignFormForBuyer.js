...
$(document).ready(function(){
	$("#returnUrl").val($('#initPath').val()+"/view/es/planform/consign/consignListForBuyer.jsp");
	//日历控件
	...
	$('#consignSave').click(function(){
		...
		{
			$.getJSON($('#initPath').val()+'/ConsignController.do?method=updateSaveConsignForBuyer&consId='+$("#objId").val(), formToJsonObject('consignForm'), function(json){
	...
	$('#consignSubmit').click(function(){
		...
		{
			$.getJSON($('#initPath').val()+'/ConsignController.do?method=updateSubmitConsignForBuyer&consId='+$("#objId").val(), formToJsonObject('consignForm'), function(json){
				if(json.result)alert(json.result);if(json.failure)return;
				//跳转到上一个页面
				$('button[name=historyBackBtn]').click();
			});
