...
$(document).ready(function(){
	...
	$("select[name$=_supplierId]").each(function(){   //设置选择供应列表
		
	})
	
//	$("select[name$=_supplierId]").change(function(){  //点击选择投标单位页面
//		var index =  $(this).attr('name').substring('0','1');
//		$("input[name="+index+"_supplierName]").val($(this).find("option:selected").html());
//		
//	})
	...
	$("#openBidGeneralVitemSave").click(function(){
		...
		$('#openbidGeneralviewForm').ajaxSubmit({
			url:$('#initPath').val()+"/OpenbidGeneralviewController.do?method=updateOpenbidGeneralviewMessage&num="+num+"&n="+$("#n").val()+"&projectId="+projectId,
			dataType:'json',
    ...
    openbidGeneralview.removeFile = function(num){
		$("#removeFileEle"+num).prev('a').remove();
		$("#removeFileEle"+num).remove();
		$("input[name="+num+"_attrId]").val("");
