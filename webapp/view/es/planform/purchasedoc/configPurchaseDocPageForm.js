...
purchaseDocDeptAudit.auditBulletin = function(){
	if(!$('#purchaseDocDeptAuditForm').valid()){
		alert("请正确填写表单！");return;
	}else if(window.confirm("确认提交?"))
	{
		$.getJSON($('#initPath').val()+'/PurchaseDocController.do?method=purchaseDocConfig&auditStatus='+$("#auditStatus").val(), formToJsonObject('purchaseDocDeptAuditForm'), function(json){
...
$(document).ready(function(){
	$('#purchaseDocDeptAuditForm').validate();
	//确认
