...
<script type="text/javascript" src="<%=request.getContextPath()%>/view/es/competitive/bidFactorInfoForm.js"></script>
<table class="tableList">
	<c:set var="i" value="0"/>
	<tr>
		<th style="width: 10%"><label><spring:message code="congruousFactorForm.factorName"/></label></th>
		<th  style="width: 10%"><label><spring:message code="congruousFactorForm.projName"/></label></th>
		<th style="width: 10%"><label><spring:message code="congFactorResponseForm.respValue"/></label></th>
		<th><label><spring:message code="congFactorResponseForm.attr"/></label></th>
	</tr>
	<c:forEach items="${factorTypeList}" var="factorType">
		<tr>
			<td colspan="4">
				<span style="font-weight: bold;margin-left: 30px">${factorType.typeName}</span>
			</td>
		</tr>
		<c:forEach items="${congruousFactorList}" var="congruousFactor">
			<c:if test="${factorType.objId == congruousFactor.factorTypeId}">
				<tr>
					<td>${congruousFactor.factorName}</td>
					<td>${congruousFactor.packName}</td>
					<td>
						<input name="respValue${i}" value="${congruousFactor.respValue}" class="required" /><span class="eleRequired">*</span>
					</td>
					<td>
	...
	<input type="hidden" name="listNum" value="${i}"/>
</table>