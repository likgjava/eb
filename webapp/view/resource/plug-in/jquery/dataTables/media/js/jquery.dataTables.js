...
(function($) {
	...
	_oExt.oStdClasses = {
		...
		"sPageButtonStaticDisabled": "disable_paginate_button",
		"sPageFirst": "first",
	     ...
	     *           node:nPaging - the DIV which contains this pagination control
			...
			"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
			{
				...
				$(nFirst).click( function () {
					if($(this).hasClass(oClasses.sPageButtonStaticDisabled))return;
			...
			"fnUpdate": function ( oSettings, fnCallbackDraw )
			{
				...
				var fnClick = function() {
					...
					oSettings._iDisplayStart = iTarget;//by wangsw = iTarget * oSettings._iDisplayLength;
					fnCallbackDraw( oSettings );
					return false;
				};
				...
				for ( i=0, iLen=an.length ; i<iLen ; i++ )
				{
					...
					anStatic = [
						...
						anButtons[anButtons.length-2], anButtons[anButtons.length-3], anButtons[anButtons.length-1]
					];
					...
					{
						anStatic[2].className += " "+oClasses.sPageButtonStaticDisabled;/*控制下一页不能点  disabled*/
						anStatic[3].className += " "+oClasses.sPageButtonStaticDisabled;/*控制下一页不能点*/
					}
					else
					{
	...
	_oExt.oSort = {
		...
		{
			var x = a?a.toLowerCase():0;/*by wangsw a.toLowerCase()改为a?a.toLowerCase():0*/
			var y = b?b.toLowerCase():0;/*by wangsw b.toLowerCase()改为b?b.toLowerCase():0*/
	 ...
	 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	$.fn.dtSelect=function(){/*by wangsw选中单行的ID*/
		return $(this).dtSelects().split(',')[0];
	}
	$.fn.dtRemoveSelect=function(){/*by wangsw选中行的删除*/
		var ids=$(this).dtSelectArray();
		var tbody=$(this).find('tbody:eq(0)');
		$.each(ids, function(i,n){
			tbody.find('tr[objId='+ n + ']').remove();
		});
	}
	$.fn.dtSelects=function(){/*by wangsw选中多行的ID,以逗号分割*/
		return $(this).dtSelectArray().toString();
	}
	$.fn.dtSelectArray=function(){/*by liangx选中多行的ID数组*/
		var selectIds=[];
		$(this).find('tbody:eq(0) > tr').each(function(){
			if($(this).hasClass('row_selected') && $(this).attr('objId') && !$(this).find('td input[name=checkAll]').attr('disabled'))
				selectIds.push($(this).attr('objId'));
		});
		return selectIds;
	}
	$.fn.dtGetRow=function(rowId,oSettings){/*by wangsw获取一行*/ 
		if(!oSettings) oSetting = $(this).dataTableSettings[0];
		var row={};var aoData=oSettings.aoData;
		for(var i=0; i<aoData.length; i++){
			if(aoData[i]._aData['objId']==rowId){ 
				row=aoData[i]._aData;break;	
			}
		}
		return row;
	}
	$.fn.dtGetRows=function(oSettings){/*by wangsw获取所有行*/
		if(!oSettings) oSetting = $(this).dataTableSettings[0];
		var rows=[];
		$.each(oSetting.aoData, function(i,n){
			rows.push(n._aData);
		});
		return rows;
	}
	$.fn.dtGetRowIdWhenEditBlur=function(){/*by wangsw 根据表格里当前td里的内容编辑完毕之后离开焦点时获取当前行的objId*/
		return $(this).parent().find('input:first:checkbox').attr('value');
	}
	$.fn.dtGetColumnPropertyWhenEditBlur=function(){/*by wangsw 根据表格里当前td里的内容编辑完毕之后获取当前编辑的属性名字,相当于类里的属性*/
		return $(this).attr('property');
	}
	$.fn.dtGetRowIdByTr=function(){/*by wangsw 根据$(tr)获取行ID*/
		return $(this).attr('objId');
	}
	$.fn.dtGetRowDataByTr=function(){/*by wangsw 根据$(tr)获取行的数据*/
		var rowId=$(this).find('input:first:checkbox[name=checkAll]').val();
		return $(this).parent().parent().dtGetRow(rowId);
	}
	...
	$.fn.dataTable = function( oInit )
	{
		...
		function classSettings ()
		{
			...
			this.oFeatures = {
				...
				"bInfo": true,
				"bAutoWidth": true,
				"bProcessing": true,/*by wangsw*/
				"bSortClasses": true,
				"bStateSave": false,
				"bServerSide": true/*by wangsw*/
			};
			...
			this.oLanguage = {
					...
					"sZeroRecords": "没有检索到相关的数据.",	 
					"sInfo": "当前显示第_START_至第_END_条,总共_TOTAL_条记录.",
					"sInfoEmpty": "",/*Showing 0 to 0 of 0 entries*/
					"sInfoFiltered": "共_MAX_页",
					"sInfoPostFix": "",
					"sSearch": "查询:",
					"sUrl": "",
					"oPaginate": {
			...
			/*
			 * Variable: sPaginationType    可选值为:full_numbers  two_button
			 * Purpose:  Note which type of sorting should be used
			 * Scope:    jQuery.dataTable.classSettings
			 */
			this.sPaginationType = "full_numbers";/*by wangsw*/
			...
			this._iDisplayStart = 1;
			this._iDisplayEnd = 15;
			
			/*by wangsw 指定列   要查询的列    和    隐藏不显示的列  自动追补ID*/
			if(oInit){
				if(oInit._iDisplayLength)	this._iDisplayLength=oInit._iDisplayLength;//指定页大小
				if(oInit.queryColumns==undefined){alert('queryColumns属性不指定你查啥?-_-');return;}
				this.singleSelect=oInit.singleSelect!=undefined?oInit.singleSelect:true;/*是否单选*/
				this.checkbox=oInit.checkbox!=undefined?oInit.checkbox:false;/*是否有复选框*/
				this.onClickCheckbox=oInit.onClickCheckbox!=undefined?oInit.onClickCheckbox:false;/*点击复选框之后追加执行的时间*/
				this.onClickAllCheckbox=oInit.onClickAllCheckbox!=undefined?oInit.onClickAllCheckbox:false;/*全选反选之后之后追加执行的事件*/
				var queryColumsArray = oInit.queryColumns.split(",");
				this.queryColumns='objId';
				for(var i=0; i<queryColumsArray.length; i++){
					if(queryColumsArray[i] != "objId")
						this.queryColumns += "," + queryColumsArray[i];	
				}
				this.hiddenColumns="";
				if(oInit.hiddenColumns){
					var hiddenColumsArray = oInit.hiddenColumns.split(",");
					for(var i=0; i<hiddenColumsArray.length; i++){
						if(hiddenColumsArray[i] != "objId")
							this.hiddenColumns += "," + hiddenColumsArray[i];
					}
				}
				...
				if(oInit.alias){
					this.alias="objId";
					var aliasArray = oInit.alias.split(",");
					for(var i=0; i<aliasArray.length; i++){
						if(aliasArray[i] != "objId")
							this.alias += "," + aliasArray[i];
					}
				}
				
				this.rightKey=oInit.rightKey;
				this.params=oInit.params!=undefined?oInit.params:{};/*追加的搜索条件*/
				this.searchZone=oInit.searchZone!=undefined?oInit.searchZone:null;/*搜索区域的form的id*/
			}
		...
		/*by wangsw*/
//		this.fnRightKey = function(){
//			$(this).bind('contextmenu', function(e) {
//				  //右键事件;	
//				  $(this).addClass('trOver');
//				  var id=this.id;
//				  var json = $(this).dtGetRowDataByTr($(this).dtGetRowIdByTr());
//				  this.rightKey(json,e);
...
//		};
		 ...
		 */
		function _fnAddData ( oSettings, aDataTemp )
		{
			...
			*/
			/*by wangsw 遍历json 构造可以显示的数据格式  定义aDataTemp是为了转化格式  定义aDataHash是为了调整顺序*/
			var aData=[];
			var colums = oSettings.queryColumns;
			if(oSettings.alias)
				colums = oSettings.alias;
			var queryColumns=colums.replace('objId,','').split(',');
			if(aDataTemp.length!=undefined)/*出来HQL查询时的解析bug*/
				aDataTemp=aDataTemp[0];
			for(var i=0; i<queryColumns.length; i++){
				var columnValue=aDataTemp[queryColumns[i]];
				if(queryColumns[i].indexOf('.')>0 && columnValue==undefined){
					var stringDotValue;
					try{
						stringDotValue=eval('aDataTemp.'+queryColumns[i]);
					}catch(e){
						stringDotValue='2222';
					}
					aData.push(stringDotValue);
				}else
					aData.push(columnValue!=undefined?columnValue:'');
			}
			...
			oSettings.aoData.push( {
				...
				"_aData": aDataTemp,/*by wangsw   aData.slice() */
				"_anHidden": [],
				"_sRowStripe": ''
			} );
		...
		function _fnDrawHead( oSettings )
		{
			...
			if ( iThs !== 0 )
			{
				...
				for ( i=0, iLen=oSettings.aoColumns.length ; i<iLen ; i++ )
				{
					...
					if ( oSettings.aoColumns[i].bVisible )
					{
						...
						if (oSettings.queryColumns.split(",")[i+1] != null )
						{
							nTh.name=oSettings.queryColumns.split(",")[i+1]
						}
					}
					else
					{
			...
			{
				//oSettings.nTable.style.width = oSettings.nTable.offsetWidth+"px"; //去掉表格的宽度，by liangxj
			}
		...
		function _fnDraw( oSettings )
		{
			...
			if ( oSettings.aiDisplay.length !== 0 )
			{
				...
				for ( var j=iStart ; j<iEnd ; j++ )
				{
					...
					/*by wangsw 给当前行的每一列的td加上name属性*/ 
					var names=oSettings.queryColumns.replace('objId,','').split(',');		
					$(aoData.nTr).attr('objId', aoData._aData.objId).find('td').each(function(i,n){
						$(this).attr('name',names[i]);
					});
					/*by wangsw 给每行的最左边加上复选框*/
					if(oSettings.checkbox){
						$(nRow).prepend('<td class="center"><input type=checkbox name=checkAll value='+aoData._aData.objId+' /></td>')/*添加复选框按钮*/
					}
					$(nRow).click(function(event){
						if( (event.target.tagName=='INPUT' && event.target.name!='checkAll') || event.target.tagName=='SELECT'){/*by wangsw*/
							return;//输入框和下拉框的点击不算选中
						}
						var inputCheckbox=$(this).find('td input:eq(0):checkbox');
						if($(this).attr('class').indexOf('row_selected')>-1){
							if($(this).attr('rowspanNum')){	
								var nowTr=$(this);var rowspanNum=parseInt($(this).attr('rowspanNum'));
								for(var j=0;j<rowspanNum;j++){/*取消选中样式*/	
									nowTr=nowTr.removeClass('row_selected').next();
								}
							}else	$(this).removeClass('row_selected');	
							inputCheckbox.attr('checked',false);
							if(oSettings.onClickCheckbox)	oSettings.onClickCheckbox(inputCheckbox);
						}else{
							if(oSettings.singleSelect){
								$('#'+oSettings.sInstance+' tbody tr').removeClass('row_selected');/*取消所有行选中*/
								$('#'+oSettings.sInstance+' tbody td input[name=checkAll]:checkbox').attr('checked',false);/*取消所有复选框选中*/
							}	
							if($(this).attr('rowspanNum')){/*选中样式*/	
								var nowTr=$(this);var rowspanNum=parseInt($(this).attr('rowspanNum'));
								for(var j=0;j<rowspanNum;j++){
									nowTr=nowTr.addClass('row_selected').next();
								}
							}else	$(this).addClass('row_selected');	
							if(!$(this).find('td input:eq(0):checkbox').attr("disabled")){
			...
			else
			{
				...
				if(oSettings.checkbox) {
					nTd.colSpan = oSettings.aoColumns.length + 1;
				}
		...
		function _fnAjaxUpdate( oSettings )
		{
			if ( oSettings.bAjaxDataGet )
			{
				...
				/*by wangsw 加了两行*/
				aoData.push( { "name": "isAJAX",     "value": true } );
				aoData.push( { "name": "queryColumns",     "value": oSettings.queryColumns } );
				aoData.push( { "name": "hiddenColumns",    "value": oSettings.hiddenColumns } );
				if(oSettings.alias && oSettings.alias.length > 0)  //如果别名不为空
					aoData.push( { "name": "alias",    		 "value": oSettings.alias + oSettings.hiddenColumns } );
				aoData.push( { "name": "sEcho",          "value": oSettings.iServerDraw } );
				aoData.push( { "name": "iColumns",       "value": iColumns } );
				aoData.push( { "name": "sColumns",       "value": _fnColumnOrdering(oSettings) } );
				/*by wangsw  iDisplayStart改为page		iDisplayLength改为rp*/
				aoData.push( { "name": "page",  "value": parseInt(oSettings._iDisplayStart)} );
				aoData.push( { "name": "rp", "value": oSettings.oFeatures.bPaginate !== false ?
					oSettings._iDisplayLength : -1 } );
				
				/*追加查询指定条件*/
				for(var key in oSettings.params){ 
					var object={name:key, value:oSettings.params[key]};
					aoData.push(object);
				}
				var fromJson=$('#'+oSettings.searchZone).serializeArray();
				for(var key in fromJson){ 
					var object={name:fromJson[key].name, value:fromJson[key].value.replace("%","%25")};
					aoData.push(object);
				}
				...
				if ( oSettings.oFeatures.bSort !== false )
				{
					...
					var queryNames = oSettings.queryColumns.split(",");
					var orderNames = "";
					var flagNames = "";
					...
					for ( i=0 ; i<iFixed ; i++ )
					{
						...
						orderNames += queryNames[oSettings.aaSorting[i][0]+1] + ",";
						flagNames += (oSettings.aaSorting[i][1] == "asc" ? true : false) + ",";
					}
					...
					/* 增加排序功能  order携带排序字段，order_flag携带排序方式  by liangxj 2010-5-17*/
					if(iFixed > 0 || iUser > 0)
					{
						aoData.push( { "name": "order",  "value": orderNames.substring(0,orderNames.length-1) } );
						aoData.push( { "name": "order_flag",  "value": flagNames.substring(0,flagNames.length-1) } );
					}
				...
				oSettings.fnServerData( oSettings.sAjaxSource, aoData, function(json) {
					if(json.failure){alert(json.result);return;}/*报了错就输出和终止 by wangsw*/
					oSettings._iDisplayStart = json.page;
		...
		function _fnAjaxUpdateDraw ( oSettings, json )
		{
			...
			_fnClearTable( oSettings );
			oSettings.appendData=json.appendData;/*by wangsw*/
			json.aaData=json.rows;json.rows=null;/*by wangsw  iTotalRecords改为total*/
			oSettings._iRecordsTotal = json.total;/*by wangsw  iTotalRecords改为total*/
			oSettings._iDisplayStart = parseInt(json.page);
			oSettings._iRecordsDisplay = json.iTotalDisplayRecords==undefined?json.aaData.length:json.iTotalDisplayRecords;/*by wangsw  iTotalRecords改为total*/
			...
			/*by wangsw	加载完之前补充左边复选框的渲染*/
			if(oSettings.checkbox && $('#'+oSettings.sInstance+' thead tr input:checkbox[name=checkAll]').length==0){
				$('#'+oSettings.sInstance+' thead tr').prepend('<th><input type=checkbox name=checkAll value="" /></th>');
				$('#'+oSettings.sInstance+' thead tr input:checkbox[name=checkAll]').click(function(){/*全选反选*/
					if(this.checked){
						$('#'+oSettings.sInstance+' tbody tr').addClass("row_selected");
						
						$('#'+oSettings.sInstance+' tbody td input:checkbox[name=checkAll]').each(function(i,n){
							if($(n).attr('disabled')){
						...
						if(oSettings.onClickAllCheckbox)	oSettings.onClickAllCheckbox($('#'+oSettings.sInstance+' tbody td input:checkbox[name=checkAll]'));
					}
					else{
						$('#'+oSettings.sInstance+' tbody tr').removeClass("row_selected");
						$('#'+oSettings.sInstance+' tbody td input:checkbox[name=checkAll]').attr('checked',false);
						if(oSettings.onClickAllCheckbox)	oSettings.onClickAllCheckbox($('#'+oSettings.sInstance+' tbody td input:checkbox[name=checkAll]'));
					}
				});
			}
			...
			/*设置表头*/
			var columnName=oSettings.checkbox?oSettings.queryColumns.split(','):oSettings.queryColumns.replace('objId,','').split(',');
			$('#'+oSettings.sInstance+' thead th').each(function(j,n){
				if(oSettings.checkbox){	
					$(this).attr('abbr',columnName[j]);
				}else 
					$(this).attr('abbr',columnName[j+1]);
			});
			...
			/*设置样式给td的class加上th的jsp样式名*/
			$('#'+oSettings.sInstance+' th').each(function(i,th){
				var obj = $('#'+oSettings.sInstance).find('tbody').find('td[name='+columnName[i]+']');
				obj.addClass($(this).attr('class').replace('sorting','').replace('sorting_asc','').replace('sorting_desc',''));
				
				$(obj).each(function(j,td){
					if($('input',td).length==0) {
						if($(td).hasClass('money')) {/*设置样式格式化金额*/
							$(td).html(formatAmount($(td).html(),2));
						}
						if($(td).hasClass('amount')) {/*设置样式格式化数量*/
							$(td).html(formatAmount($(td).html()));
						}
						if($(td).hasClass('date')) {/*设置date格式化*/
							$(td).html($(td).html().substring(0,10));
						}
						if($(td).hasClass('datetime')) {/*设置datetime格式化*/   
							$(td).html($(td).html().substring(0,19));
						}
						if($(td).hasClass('omission')) {/*设置样式格式化单行溢出*/
							$(td).attr("title",$(td).html());
							var length = 8;
							if($(th).attr("omiLength")) {
								length = $(th).attr("omiLength");
							}
							$(td).html(formatOmission($(td).html(),length));
						}
					}
				})
			});
			...
			/*by wangsw添加右键菜单事件*/
			if(oSettings.rightKey){/*如果注册了右键菜单事件*/
				$('#'+oSettings.sInstance+' tbody tr').each(function(){
					$(this).bind('contextmenu', function(e) {
						  $(this).addClass('trOver');
						  var id=this.id;
						  var json = $(this).dtGetRowDataByTr($(this).dtGetRowIdByTr());
						  oSettings.rightKey(json,e);
						  $(this).removeClass('trOver');
						  return false;
					});
				});
		...
		function _fnSort ( oSettings, bApplyClasses )
		{
			...
			{
				oSettings.aiDisplay = oSettings.aiDisplayMaster.slice();
				oSettings._iDisplayStart = 1; /* reset display back to page 0 */
				_fnCalculateEnd( oSettings );
				_fnDraw( oSettings );
			}
		}
		...
		function _fnSortingClasses( oSettings )
		{
			...
			for ( i=0 ; i<oSettings.aoColumns.length ; i++ )
			{
				if ( oSettings.aoColumns[i].bSortable )
				{
					...
					/* If has operation class ，don't add sort class to the header ,and take bSortable false    by liangxj*/
					if(!$(oSettings.aoColumns[i].nTh).hasClass(oSettings.oClasses.sOperation)){
		...
		function _fnPageChange ( oSettings, sAction )
		{
			...
			{
				oSettings._iDisplayStart = oSettings._iDisplayStart - 1;
			...
			else if ( sAction == "next" )
			{
				...
				if ( parseInt(oSettings._iDisplayStart) * oSettings._iDisplayLength > oSettings.fnRecordsTotal() )
				{	 
					oSettings._iDisplayStart = Math.ceil((oSettings.fnRecordsTotal()) / oSettings._iDisplayLength);
				}
			}
			else if ( sAction == "last" )
			{
				if ( parseInt(oSettings._iDisplayLength) >= 0 )
				{	 
					oSettings._iDisplayStart = Math.ceil((oSettings.fnRecordsTotal()) / oSettings._iDisplayLength);
				}
				else
...
//					(!oSettings.fnRecordsTotal())?oSettings.oLanguage.sInfoFiltered.replace('共_MAX_页',""):oSettings.oLanguage.sInfoFiltered.replace('_MAX_', 
//						oSettings.fnRecordsTotal())+ oSettings.oLanguage.sInfoPostFix;
//			}
//			else if ( oSettings.fnRecordsDisplay() == oSettings.fnRecordsTotal() )
//			{	
//				var html;
//				var selectLength=$('select', nFirst).length, selectValue=$('select', nFirst).val();
//				if(selectLength!=0)
//					html=' 每页<span>'+$('span', nFirst).clone(true).html()+'</span>条';
//				else html=_fnFeatureHtmlLength(oSettings).innerHTML;/*by wangsw顶上的每页大小下移*/
//			 
//				/* Normal record set */
//				if(oSettings.bPaginate)/*by wangsw*/
//				nFirst.innerHTML = oSettings.oLanguage.sInfo.
//						replace('_START_',oSettings._iDisplayStart+1).
//						replace('_END_',oSettings.fnDisplayEnd()).
//						replace('_TOTAL_',oSettings.fnRecordsDisplay())+ html +
//					oSettings.oLanguage.sInfoPostFix;
//				
//				/*by wangsw*/
//				if(selectLength!=0)	$('span select', nFirst).val(selectValue);
//				iPushFeature = 1;
//				$('select', nFirst).change(function(){
//					_fnChangeLength(this, oSettings);
//				});
//			}
//			else
//			{	 
...
//				if(oSettings.oFeatures.bPaginate){/*by wangsw*/
//					var str1=oSettings.oLanguage.sInfo.replace('_START_',oSettings._iDisplayStart).replace('_END_',oSettings.fnDisplayEnd()-1).replace('_TOTAL_',oSettings.fnRecordsTotal()) 
//					var str2=' '+ (!oSettings.fnRecordsTotal())?oSettings.oLanguage.sInfoFiltered.replace('共_MAX_页',""):oSettings.oLanguage.sInfoFiltered.replace('_MAX_', oSettings.fnRecordsTotal())+ html + oSettings.oLanguage.sInfoPostFix;
//					nFirst.innerHTML = str1+str2;
				var totalNum=$('#'+oSettings.sInstance+'_paginate .totalNum');
				totalNum.html('共#PN页/#RN条记录'.replace('#PN',Math.ceil((oSettings.fnRecordsTotal()) / oSettings._iDisplayLength)).replace('#RN',oSettings.fnRecordsTotal()));
//				}
				/*by wangsw顶上的每页大小下移, 并添加事件*/
				if(selectLength!=0)	$('span select', nFirst).val(selectValue);
				iPushFeature = 1;
				$('select', nFirst).change(function(){
					_fnChangeLength(this, oSettings);
...
//			}
		...
		function _fnFeatureHtmlLength ( oSettings )
		{
			...
			$('select', nLength).change(function(e){/*by wangsw*/
				_fnChangeLength(this, oSettings);
			});
		...
		/*by wangsw*/
		function _fnChangeLength(thiss, oSettings) {
			var iVal = $(thiss).val();
