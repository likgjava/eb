			...
			switch (intKey) {
				...
				default:
					...
					if (document.getElementById(strBoxId) == null) {
						...
						$(objBox).width(objSel.width()+70).attr('type', 'text').attr('id', strBoxId).attr('autocomplete', 'off').attr('maxlength', defaults.maxlength).addClass(objSel.attr('class'));
						...
						//by wangsw 5
						var dataStr='';
						$.each(options.datas.result,function(i,n){
							dataStr+=n.name+' ';
						});
						$('#'+strBoxId).autocomplete(dataStr.split(" "),{autoFill: true,matchContains: true});//autoFill: true,matchContains: false
						$('#'+strBoxId).result(function(event,data,formatted){ 
							$(objSel).find('option').each(function(i,n){
								if(n.text==data){ 
									n.selected=true;
									$(objSel).change();
									$(objBox).trigger('blur');
...
//								$(objBox).trigger('blur');//by wangsw 
			...
			$('#' + strBoxId).bind('blur', function() {
				//create new option?
				var bolCreateOption	=	false;//by wangsw
