...
ProcessDefNodeFormList.success = function(){
	$("#processDefNodeFormGrid").flexAddOptionStr({
		'<span><a href="#" class="abtn">修改</a>&nbsp;&nbsp;&nbsp;&nbsp;</span>' : function(btn,rowId,obj){
			btn.click(function(){
				$.epsDialog({
			        ...
			        title:'修改流程节点审核表单',
			        url:$("#initPath").val()+'/ProcessDefNodeFormTemplateController.do?method=toprocessDefNodeFormFormView&objId='+rowId+'&nodeId='+$('#processDefNodeId').val(),
			        width: '800',
			        height: '500',
			        maxWin:false,
			        onClose: function(){
		...
		'<span><a href="#" class="abtn">删除</a></span>' : function(btn,rowId,obj){
			btn.click(function(){
				$.getJSON($('#initPath').val()+'/ProcessDefNodeFormTemplateController.do?method=removeProcessDefNodeForm',{'objId':rowId}, function(json){
...
ProcessDefNodeFormList.add = function () {
	$.epsDialog({
		id:'process_node_form_form',
        title:'新增流程节点审核表单',
        url:$("#initPath").val()+'/ProcessDefNodeFormTemplateController.do?method=toprocessDefNodeFormFormView&nodeId='+$('#processDefNodeId').val(),
        width: '800',
        height: '500',
        maxWin:false,
        onClose: function(){
...
<div class="partContainers">
	<flex:flexgrid checkbox="false" id="processDefNodeFormGrid" url="ProcessDefNodeFormTemplateController.do?method=list&processDefNode.objId=${param.processDefNodeId}&order=formPropertyNo" 
