...
//审核
OrgInfoAuditList.toAudit = function(orgInfoId,agencyId) {
	/** 小额交易-机构审核*/
	$('#conBody').loadPage($('#initPath').val()+'/OrgInfoController.do?method=toAuditOrgInfo&orgInfoId='+orgInfoId+"&auditRole=AGENCY");
	
	/**协议供货-机构审核
	var url = '';
	if(agencyId != "undefined" && agencyId != "") {
		url = $('#initPath').val()+'/OrgInfoController.do?method=toAuditOrgInfo_XYGH&orgInfoId='+orgInfoId+"&auditRole=AGENCY";
	}else {
		url = $('#conBody').loadPage($('#initPath').val()+'/OrgInfoController.do?method=toAuditOrgInfo&orgInfoId='+orgInfoId+"&auditRole=AGENCY");
	}
...
//查看
OrgInfoAuditList.view = function(orgInfoId,agencyId) {
	/**小额交易,查看页面*/
...
$(document).ready(function() {
	//设定返回路径
	$("#returnUrl").val("view/bizplatform/agency/agency/agency_audit_list.jsp");
	...
	OrgInfoAuditList.oTable = $('#OrgInfoAuditList').dataTable({
		...
		'fnRowCallback' : function(nRow, aData, iDisplayIndex) {
			// 添加操作按钮
			$(nRow).append('<td align="center"><a href="javascript:void(0);" onclick="OrgInfoAuditList.view(\''+aData.objId + '\',\''+aData.agencyId+'\');return false;">查看</a><a href="javascript:void(0);" onclick="OrgInfoAuditList.toAudit(\''+aData.objId + '\',\''+aData.agencyId+'\')">审核</a></td>')
			return nRow;
		},
		"sAjaxSource" : $('#initPath').val()+ "/OrgInfoController.do?method=listOrgInfoForAudit&type=AGENCY",
