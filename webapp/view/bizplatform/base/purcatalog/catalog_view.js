...
catalogForm.audit = function(auditType){
	$.getJSON($('#initPath').val()+'/PurCatalogController.do?method=auditPurCatalog',
			{"objId":$("#objId").val(),"auditType":auditType},
			function(json){
				if(json.success){
					alert(json.result);
...
catalogForm.initsaveForm = function(){
	...
	$("#CatalogDetailForm").find("ul:last").empty().append('<li id="1" class="fullLine"><label>采购方式：</label><label>采购金额（元）：</label></li>');
...
$(document).ready(function(){
	...
	$("#auditReturn").click(function(){				
		$('#conBody').loadPage($('#initPath').val()+"/view/bizplatform/base/purcatalog/catalog_list.jsp");
	})
