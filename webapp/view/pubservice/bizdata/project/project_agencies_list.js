...
ProjectList.openOperatorPage=function(type,objId){
	if("detail" == type){
		$("#returnUrl").val($('#initPath').val()+"/view/pubservice/bizdata/project/project_agencies_list.jsp");
	...
	ProjectList.oTable = $('#projectManageList').dataTable({
		...
		'fnRowCallback' : function(nRow, aData, iDisplayIndex) {
			...
			if($(ProjectList.oTable.dataTableSettings).attr("params")['flag']=='down'){//查看、评价
				$(nRow).append('<td  align="center"><a  href="javascript:void(0);" onclick="ProjectList.openOperatorPage(\'detail\',\''+aData.objId+'\');return false;">查看</a><a href="javascript:void(0);" onclick="ProjectList.openEvaluateDiv(\''+aData.objId+'\',\''+aData.projName+'\')">评价</a></td>')
			}else{//查看
		...
		"sAjaxSource" : $('#initPath').val()+ "/ProjectController.do?method=toAgenciesList",
		"params":{"flag":"doing"},
