...
ProjectList.openOperatorPage=function(type,objId){
	if("detail" == type){
		$("#returnUrl").val($('#initPath').val()+"/view/pubservice/bizdata/project/project_buyer_list.jsp");
	...
	$.getJSON($("#initPath").val()+"/ProjectController.do?method=getEvaluateObject",{projectId:projectId},function(json){
		if(json.success){
			...
			$.each(json.evaluateOrg,function(index,obj){
				ahref += '&nbsp;<a id="'+obj.objId+'" href="javascript:void(0);" onclick="ProjectList.openEvaluateClick(\''+obj.objId+'\',\''+projectId+'\',\''+projectName+'\');return false;">'+obj.orgName+'</a>&nbsp;';
			})
			var evaluateDivHtml = '<div class="formLayout form2Pa"><h4><span>&nbsp;</span></h4><ul>'+
		    '<li class="fullLine">'+ahref+'</li></ul>'+
			'<div class="operationBtnDiv"><button id="evlautaeDivBtn" type="button"  class="largeBtn"><span>关闭</span></button></div></div>';
...
$(document).ready(function(){
	...
	ProjectList.oTable = $('#projectManageList').dataTable({
		...
		'fnRowCallback' : function(nRow, aData, iDisplayIndex) {
			//添加操作按钮 alert(JSON.stringify(aData.objId.projCode));
		...
		"sAjaxSource" : $('#initPath').val()+ "/ProjectBuyerController.do?method=toBuyerList",
		"params":{"flag":"doing"},
